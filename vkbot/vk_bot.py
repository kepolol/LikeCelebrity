...
def bot_response(message, user_id):
    if message == "Ты ошибся...":
        update_feedback("Bad")
        return "Попробуй отправить другое фото."
    elif message == "Да это я! Похож!":
        update_feedback("Good")
        return "Отлично!"
    elif message == "":
        return "Я могу обработать только одну фотографию."
    ...
    elif message == "Покажи статистику отзывов":
        stat = get_statistics()
        good = "количество положительных отзывов: {}.\n".format(stat[0])
        bad = "Но в {} случаях пользователи посчитали, что я ошибся.".format(stat[1])
        return "На данный момент " + good + bad
    else:
...
keyboard.add_button('Ты ошибся...', color=VkKeyboardColor.NEGATIVE)
keyboard.add_button('Да это я! Похож!', color=VkKeyboardColor.POSITIVE)
        ...
        if attach_data:
            if len(attach_data) == 2:
                if attach_data["attach1_type"] == "photo":
                    write_msg(event.user_id, "Фото принято, обрабатываю...")
                    ...
                    resp = "Ты пытаешься мне отправить что то не то...\nОтправь мне свою фотографию."
                    write_msg(event.user_id, resp)
                    dur_time = time.time() - start
                    add_log(time.asctime(), event.user_id, "Отправленное вложение не является фото", resp, dur_time)
            else:
                resp = bot_response(event.message, event.user_id)
                write_msg(event.user_id, resp)
                dur_time = round(time.time() - start, 4)
                add_log(time.asctime(), event.user_id, "Было отправлено более одного вложения", resp, dur_time)
        else:
